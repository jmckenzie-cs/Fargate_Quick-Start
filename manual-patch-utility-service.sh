...
{ 
    ...
    # Get the used task definition for the service
    ...
    if [ "$ARCH" == "arm64" ]; then
        ...
        -falconctl-opts "--trace=debug" \
        -ecs-spec "$JSON_STRING" > $selected_service/${task_def_name}-patched.json
    else
    ...
    echo "A new task definition revision has been created and saved on: $OUTPUT_DIR/$selected_service/${task_def_name}-patched.json created"
